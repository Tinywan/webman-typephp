...
CODE;
        ...
        if (!empty($cfg['sources'])) {
            ...
            foreach ($sources as $sourceEntry) {
                ...
                if (is_file($realPath)) {
                    ...
                    $this->climate->out("[Scanning {$sourceIndex}/{$totalSources}] {$src} ...");
    ...
    protected function doConvert(string $phpCode): string
    {
        ...
        foreach ($stmts as $v) {
            if ($v instanceof Node\Stmt\Declare_) {
                ...
                // Ignore top-level stray execution expressions (such as initialization calls)
                // during static compilation; these are executed at runtime by entrypoint/main.php
    ...
    protected function parseNamespace(Node\Stmt\Namespace_ $node): string
    {
        ...
        foreach ($node->stmts as $v2) {
            ...
            } elseif ($v2 instanceof Node\Stmt\Expression) {
                // Ignore namespace-level stray execution expressions (such as initialization calls)
                // during static compilation; these are executed at runtime by entrypoint/main.php
