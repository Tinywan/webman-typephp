...
it('keeps Webman plugin configuration available for runtime discovery', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->discoverPluginConfigResources())
            ->toBe([
...
it('selects the PHP 8.4 intl polyfill declarations for AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateFlattenedSources())
            ->toBe([
        ...
        expect($idn)
            ->toContain('const IDNA_DEFAULT = 0;')
            ->toContain('function idn_to_ascii(?string $domain');
        expect(str_contains($idn, 'defined('))
            ->toBeFalse()
            ->and(str_contains($idn, 'function_exists'))
            ->toBeFalse();
        $normalizer = (string) file_get_contents(
            $directory . '/.typephp/build/symfony-normalizer-functions.php',
        );
        expect($normalizer)
...
it('selects the PHP 8.4 branch of the PHP 8.5 polyfill for AOT', function (): void {
    ...
    try {
        ...
        foreach (['function_exists', 'PHP_VERSION_ID', 'extension_loaded', 'locale_is_right_to_left', 'ValueError'] as $absent) {
...
it('rewrites fall-through switch cases into terminal AOT sources', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/symfony/cache/Traits/ValueWrapper.php',
        "<?php\nclass \xA9\n{\n}\n",
    );
    file_put_contents(
...
it('moves Monolog browser style caches out of captured static locals for AOT', function (): void {
    ...
    try {
        ...
        $generated = (string) file_get_contents(
            $directory . '/.typephp/build/monolog-browser-console-handler.php',
        );
        expect($generated)
...
it('folds PhpZip interface constants used by AOT defaults', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/nelexa/zip/src/ZipFile.php',
        <<<'PHP'
        <?php
        class ZipFile {
...
it('expands CarbonInterval variable variables in its AOT copy', function (): void {
    ...
    try {
        ...
        expect($generated)
            ...
            ->not->toContain('CarbonInterface::ONE_DAY_WORDS')
            ->toContain('foreach ($interval as $partIndex => &$item)')
            ->toContain('$partIndex, $actualParts')
            ->toContain('foreach ($diffIntervalArray as $skipIndex => &$unitData)')
            ->toContain('$nextIndex = $skipIndex + 1;')
            ->toContain('$humanOptions = $this->getForHumansInitialVariables')
            ->toContain('$translator = $humanOptions[\'translator\'] ?? null;')
            ->not->toContain('extract(')
            ->toContain('while ($partValues = array_shift($parts))')
            ->toContain('$unit = $partValues[2];')
            ->not->toContain('while ([$part, $value, $unit]')
            ->toContain("'years' => \$years")
            ->toContain("if (\$target === 'dayz')")
            ->not->toContain('$$');
    } finally {
        removeTypephpTestDirectory($directory);
    }
...
it('moves Webman database loading side effects into the AOT entrypoint', function (): void {
    ...
    try {
        ...
        expect(file_get_contents($directory . '/.typephp/build/webman-database-model.php'))
            ->not->toContain('require_once __DIR__');
        ...
        expect(file_get_contents($directory . '/.typephp/build/webman-database-initializer.php'))
            ->not->toContain('Initializer::init(');
...
it('clears Webman database grammar without Closure call in AOT copies', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/webman/database/src/DatabaseManager.php',
        <<<'PHP'
<?php
...
it('clears Webman ThinkORM state without Closure call in AOT copies', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/webman/think-orm/src/DbManager.php',
        <<<'PHP'
<?php
class DbManager {
...
it('keeps the Workerman parallel barrier in a reference-capable AOT slot', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/workerman/coroutine/src/Parallel.php',
        <<<'PHP'
<?php
class Parallel {
...
it('keeps the Workerman websocket request header separate from response headers', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/workerman/workerman/src/Protocols/Websocket.php',
        <<<'PHP'
<?php
class Websocket {
...
it('keeps the ip2region integer slot stable on 32-bit fallback', function (): void {
    ...
    mkdir($directory . '/vendor/zoujingli/ip2region', 0777, true);
    file_put_contents(
        $directory . '/vendor/zoujingli/ip2region/XdbSearcher.php',
        <<<'PHP'
<?php
class XdbSearcher {
...
it('keeps the Webman monitor iterator object-typed in its AOT copy', function (): void {
    ...
    file_put_contents(
        ...
        . "        } else {\n            \$iterator = new RecursiveIteratorIterator(\$monitorDir);\n        }\n"
    ...
    try {
        ...
        expect(file_get_contents($directory . '/.typephp/build/app-process-monitor.php'))
            ->toContain('$iterator = new \ArrayIterator([new SplFileInfo($monitorDir)]);')
...
it('rewrites typed-reference captures into AOT sources', function (): void {
    ...
    try {
        ...
        expect($curlFactory)
            ...
            ->not->toContain('$collectingTrailers = false;')
            ->toContain('use (&$startingResponse, &$collectingTrailers)');
        ...
        expect($promisesUtils)
            ...
            ->toContain('if (!isset($rejections)) {')
            ->toContain('use ($recursive, $promises)');
...
it('expands Postgres DSN extract into explicit configuration reads', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/illuminate/database/Connectors/PostgresConnector.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        ...
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-database-postgres-connector.php']);
        $source = (string) file_get_contents(
            $directory . '/.typephp/build/illuminate-database-postgres-connector.php',
        );
        expect($source)
            ->not->toContain('extract(')
...
it('declares the optional Postgres JSON path quote argument for AOT', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/illuminate/database/Query/Grammars/PostgresGrammar.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-database-postgres-grammar.php']);
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-database-schema-blueprint.php']);
...
it('rejects nonempty Illuminate dynamic require data explicitly for AOT', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/illuminate/filesystem/Filesystem.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-filesystem.php']);
...
it('declares Illuminate HTTP Response exception callbacks explicitly for AOT', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/illuminate/http/Client/Response.php',
        <<<'PHP'
<?php
class Response
{
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-http-client-response.php']);
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/symfony-http-foundation-request.php']);
        ...
        expect($source)
            ...
            ->not->toContain('$host = $this->getTrustedValues')
...
it('replaces Symfony RequestStack closure binding with the AOT reset bridge', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/RequestStack.php',
        <<<'PHP'
<?php
class RequestStack
{
...
it('makes Symfony ResponseHeaderBag partitioned cookies explicit in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ResponseHeaderBag.php',
        <<<'PHP'
<?php
...
it('keeps Symfony PDO session payloads type-stable through an AOT holder', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/PdoSessionHandler.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        ...
        expect(substr_count($content, '$dataHolder = new \stdClass();'))->toBe(2)
            ->and($content)->toContain('fwrite($dataHolder->value, $sessionData)')
...
it('keeps Symfony controller strings separate from resolved callables for AOT', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ControllerResolver.php',
        <<<'PHP'
<?php
...
it('makes Symfony exception log channels explicit in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ErrorListener.php',
        <<<'PHP'
<?php
class ErrorListener
{
...
PHP,
    ...
    try {
        ...
        expect($content)
            ...
            ->not->toContain('func_get_arg')
            ->not->toContain('func_num_args')
            ->toContain('$reflectionClass = new \ReflectionClass($class);')
            ->toContain('$interfaceReflection = new \ReflectionClass($interface);')
...
it('avoids Symfony private session container shadowing in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/SessionListener.php',
        <<<'PHP'
<?php
...
it('uses the stable Symfony main request value in the AOT interface default', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/HttpKernelInterface.php',
        <<<'PHP'
<?php
...
it('avoids Symfony private MIME encoder shadowing in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ParameterizedHeader.php',
        <<<'PHP'
<?php
...
it('keeps ThinkORM relation result mutable across array and model states for AOT', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ModelRelationQuery.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        ...
        expect($content)
            ->toContain('resultToModel(mixed &$result)')
            ->not->toContain('resultToModel(array &$result)');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
PHP,
    ...
    try {
        ...
        expect(substr_count($content, 'transaction(function ($connection) use'))->toBe(4)
            ->and($content)->toContain('$untouched = function () {}')
...
it('keeps ThinkORM result set mutable across array and collection states for AOT', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ResultOperation.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        ...
        expect($content)
            ->toContain('resultSet(mixed &$resultSet')
            ->not->toContain('resultSet(array &$resultSet');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
it('keeps ThinkORM insert IDs type-flexible through an AOT state holder', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/PDOConnection.php',
        <<<'PHP'
<?php
class PDOConnection
{
...
PHP,
    ...
    try {
        ...
        expect($content)
            ...
            ->not->toContain('$pk          = count($pk) > 1')
            ->toContain('$insertIdState = new \stdClass()')
            ->toContain('$insertIdState->value = (int) $insertId')
            ->toContain('$insertIdState->value = (float) $insertId')
            ->toContain('return $insertIdState->value')
            ->not->toContain('$insertId = (int)')
            ->toContain('$dbMasterState = new \stdClass()')
            ->toContain('$dbMasterState->value = []')
            ->toContain('$dbMasterState->value[$name] =')
            ->toContain('$dbMasterState->value)')
...
it('uses a static ThinkORM query class for the AOT instanceof check', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/WhereQuery.php',
...
it('matches ThinkORM dynamic parser calls to their AOT arity', function (): void {
    ...
    try {
        ...
        expect($content)
            ...
            ->toContain('[\'parseCompare\', \'parseBetween\', \'parseIn\', \'parseExp\', \'parseNull\', \'parseBetweenTime\', \'parseTime\', \'parseExists\', \'parseColumn\']')
...
PHP;
    ...
    try {
        ...
        file_put_contents(
            $sourceDirectory . '/ReflectionCache.php',
            str_replace('// 反射逻辑', '// upstream changed', $source),
        );
        expect(fn(): array => $generator->generateSwitchTerminalSources())
            ->toThrow(
                RuntimeException::class,
...
PHP;
    ...
    try {
        ...
        file_put_contents(
            $sourceDirectory . '/LoginController.php',
            str_replace('captcha() : Response', 'captcha(Request $request) : Response', $source),
        );
        expect(fn(): array => $generator->generateSwitchTerminalSources())
            ->toThrow(
                RuntimeException::class,
...
it('passes integral pixel coordinates to GD in the Webman captcha AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/CaptchaBuilder.php',
        '<?php return imagecolorat($image, $x, $y);',
    );
...
it('keeps Symfony uploaded filesize parsing type-stable for AOT', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/symfony/http-foundation/File/UploadedFile.php',
        <<<'PHP'
<?php
class UploadedFile
{
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/symfony-http-foundation-uploaded-file.php']);
...
it('rejects Illuminate Redis custom closure rebinding explicitly for AOT', function (): void {
    ...
    file_put_contents(
        $directory . '/vendor/illuminate/redis/RedisManager.php',
        <<<'PHP'
<?php
use Closure;
class RedisManager
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-redis-manager.php']);
...
it('uses the explicit owner for Carbon inherited default constants in AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-interface.php']);
...
it('folds Illuminate password facade contract constants in its AOT copy', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-support-password-facade.php']);
...
it('rejects Illuminate custom instance closure rebinding explicitly for AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-support-multiple-instance-manager.php']);
        $source = (string) file_get_contents($directory . '/.typephp/build/illuminate-support-multiple-instance-manager.php');
        expect($source)
...
it('defers Illuminate Sleep static reference storage in its AOT copy', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-support-sleep.php']);
...
it('keeps Illuminate excerpt strings separate from fluent objects in AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-support-str.php']);
...
it('keeps Carbon dynamic unit property values type-stable for AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-date.php']);
        ...
        expect($source)
            ...
            ->not->toContain('->bindTo(')
            ->not->toContain('Closure::bind(')
            ->not->toContain('$boundMacro')
            ->toContain('$normalizedValue = self::monthToInt($value, $name);')
            ->toContain('case \'year\': $year = $normalizedValue; break;')
            ->toContain('case \'second\': $second = $normalizedValue; break;')
            ->not->toContain('$$name')
...
it('expands Carbon boundary magic units into declared AOT calls', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-boundaries.php']);
...
it('keeps the Carbon rounding parameter separate from range keys in AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-rounding.php']);
...
it('uses declared Carbon comparison methods in the AOT copy', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-comparison.php']);
...
it('fails explicitly for Carbon interval trait mixin result copying in AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-mixin.php']);
...
it('keeps Carbon debug formatting dynamic in the Options AOT copy', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-options.php']);
...
PHP;
    file_put_contents(
        $directory . '/vendor/nesbot/carbon/src/Carbon/Traits/Localization.php',
        $fixture,
    );
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-traits-localization.php']);
...
it('flattens the pinned Carbon DatePeriod base class guard for AOT ordering', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/carbon-date-period-base.php']);
        $source = (string) file_get_contents($directory . '/.typephp/build/carbon-date-period-base.php');
        expect($source)
            ->toContain('class DatePeriodBase extends DatePeriod')
            ->not->toContain('if (!class_exists(');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
it('initializes the Eloquent aggregate alias slot explicitly for AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-eloquent-queries-relationships.php']);
...
it('expands the CarbonInterval milliseconds magic factory for AOT', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateSwitchTerminalSources())
            ->toBe(['.typephp/build/illuminate-support-interacts-with-time.php']);
...
it('defers the Illuminate Collection unique reference storage', function (): void {
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateRefCaptureSources())
            ->toBe(['.typephp/build/illuminate-collections-collection.php']);
...
it('expands the LazyCollection sliding tap before yielding its window', function (): void {
    ...
    mkdir($directory . '/vendor/illuminate/collections', 0777, true);
    file_put_contents(
        $directory . '/vendor/illuminate/collections/LazyCollection.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        $generator = new ProjectGenerator($directory);
        expect($generator->generateRefCaptureSources())
            ->toBe(['.typephp/build/illuminate-collections-lazy-collection.php']);
        $source = (string) file_get_contents(
            $directory . '/.typephp/build/illuminate-collections-lazy-collection.php',
        );
        expect($source)
...
PHP;
    ...
    file_put_contents(
        $traits . '/RedisCluster61ProxyTrait.php',
        str_replace(
            ['Redis61ProxyTrait', "'>='"],
...
PHP);
    ...
    try {
        ...
        expect($aot)
            ->toContain('return $vars[0];')
            ->not->toContain('$k = 0;')
            ->not->toContain('return $vars[$k];');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
<?php
class PHPMailer {
    public function encodeHeader(int $matchcount): string {
        if ($matchcount > 0) {
            $encoding = 'Q';
        } else {
            $encoding = false;
...
PHP);
    ...
    try {
        ...
        expect($generated)
            ->toContain('$v1Parts = explode')
            ->toContain('$v2Parts[$i]')
            ->not->toContain('$v1 = explode');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
it('keeps Symfony output chunk widths separate from line indexes in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/OutputFormatter.php',
        <<<'PHP'
<?php
...
it('makes the Symfony progress finished indicator explicit in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/ProgressIndicator.php',
        <<<'PHP'
<?php
...
it('keeps Symfony question input sentinels separate from string answers in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/QuestionHelper.php',
        implode("\n", [
            '<?php',
            '            $ret = false;',
...
it('avoids Symfony style private output property shadowing in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/SymfonyStyle.php',
        implode("\n", [
            '<?php',
...
it('avoids a reference to an unkeyed Symfony listener append slot in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/EventDispatcher.php',
        <<<'PHP'
<?php
...
it('preserves Symfony lazy listener arrays in its AOT interface copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/EventDispatcherInterface.php',
        implode("\n", [
            '<?php',
...
it('separates Symfony accept header attributes from its promoted string value in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/AcceptHeaderItem.php',
        <<<'PHP'
<?php
...
PHP,
    ...
    try {
        ...
        expect($content)
            ->toContain('as $name => $attributeValue')
            ->toContain('setAttribute($name, $attributeValue)');
    } finally {
        removeTypephpTestDirectory($directory);
    }
});
...
it('separates Symfony query input from its parsed array result in its AOT copy', function (): void {
    ...
    mkdir($sourceDirectory, 0777, true);
    file_put_contents(
        $sourceDirectory . '/HeaderUtils.php',
        <<<'PHP'
<?php
        $query = [];
...
PHP,
        ...
        'vendor/symfony/console/Output/ConsoleSectionOutput.php' => '<?php $this->lines -= (int) ceil($this->getDisplayLength($lastLine) / $width) ?: 1;',
        'vendor/symfony/http-foundation/BinaryFileResponse.php' => '<?php $this->maxlen = $end < $fileSize ? $end - $start + 1 : -1;',
